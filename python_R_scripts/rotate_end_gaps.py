...
"""
Rotate an MSA so that any trailing gaps at the END of the reference sequence
are moved to the start of the alignment, then remove all gaps.

This script ONLY handles gaps at the END of the reference.
"""
...
def count_end_gaps(seq):
    """Return number of trailing gaps at end of sequence."""
    s = str(seq)
    i = 0
    while i < len(s) and s[len(s) - 1 - i] == "-":
        i += 1
    return i


def rotate_alignment_right(alignment, shift):
    """Rotate alignment columns right by shift."""
...
def main(input_fasta, reference_id, output_fasta):
    ...
    end_gaps = count_end_gaps(ref.seq)

    records = alignment
    if end_gaps > 0:
        records = rotate_alignment_right(records, end_gaps)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Rotate MSA based on trailing gaps at end of reference sequence"
    )
